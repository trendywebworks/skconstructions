<div class="row">                
    <div class="col-xl-5 col-lg-5 col-sm-5 layout-spacing">
        <div class="statbox widget box box-shadow">

        <form>
            <div class="row">
                <div class="col-xs-12 col-md-6">         
                    <div class="form-group mb-4">
                            <label for="date">Date</label>
                            <div id="datepicker" 
                        class="input-group date" 
                        data-date-format="dd-mm-yyyy">
                        <input class="form-control" 
                            type="text" readonly />
                        <span class="input-group-addon"></span></div>
                    </div>
                </div>

                <div class="col-xs-12 col-md-6">
                    <div class="form-group mb-4">
                        <label for="particular">Party Name</label>
                        <select class="form-select mb-3" id="contact-person">
                            <option>Sumit & Groups</option>
                            <option selected="">Sameer traders </option>
                            <option>Bhupesh Construction </option>
                        </select>
                    </div>
                </div>

                <div class="col-xs-12 col-md-6">
                        <div class="form-group mb-4">
                        <label for="particular">Loan Amount</label>
                        <input type="number" class="form-control">
                    </div>
                </div>
                
                <div class="col-xs-12 col-md-6">
                        <div class="form-group mb-4">
                        <label for="particular">Interest Per/Month</label>
                        <input type="number" class="form-control">
                    </div>
                </div>

                <div class="col-xs-12 col-md-6">
                    <div class="form-group mb-4">
                        <label for="particular">Total Amount</label>
                        <input class="form-control" id="disabledInput" type="text" placeholder="" disabled>
                    </div>
                </div>

                <div class="col-xs-12 col-md-6">
                    <div class="form-group mb-4">
                        <label for="exampleFormControlTextarea1">Remarks</label>
                        <textarea class="form-control" id="exampleFormControlTextarea1" rows="3"></textarea>
                    </div>
                </div>

            </div>
            <input type="submit" name="time" class="mt-4 mb-4 btn btn-primary btn-lg">
        </form>

        </div>
    </div>
</div>