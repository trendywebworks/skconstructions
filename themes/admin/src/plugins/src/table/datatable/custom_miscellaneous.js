...
"<'dt--bottom-section d-sm-flex justify-content-sm-between text-center'<'dt--pages-count  mb-sm-0 mb-3'i><'dt--pagination'p>>",
    ...
    */
    ...
    var dttable = $('#html5-extension').DataTable( {
        "dom": "<'dt--top-section'<'row'<'col-sm-12 col-md-6 d-flex justify-content-md-start justify-content-center'B><'col-sm-12 col-md-6 d-flex justify-content-md-end justify-content-center mt-md-0 mt-3'f>>>" +
    "<'table-responsive'tr>" +
    "<'dt--bottom-section d-sm-flex justify-content-sm-between text-center'<'dt--pages-count  mb-sm-0 mb-3'i><'dt--pagination'p>>",
	        buttons: {
	            buttons: [
	                ...
	                { extend: 'pdf', className: 'btn btn-sm sk-dt-export-btn', exportOptions: {
	                        columns: ':not(:last)'
	                    },
	                    footer: true 
	                }
    ...
    $(api.table().footer()).find('th, td').each(function () {
        $(this).html('');
    });
            ...
            if($('.sumcol').length > 0)
            {
                ...
                $('.sumcol').each(function(i, obj) 
                {
                    ...
                    $( api.column( iid ).footer() ).html('₹'+total.toFixed(2)); 
                });  
                
                if($('.text-empty').length > 0){
                    $('.text-empty').each(function() {
                        $(this).html(''); // Just clear the cell directly
                    });
                }
            }
        }
    ...
    //     $('.sumcol').each(function(i, obj) {
    //         var iid = $(this).data('id');
    //         var csum = dttable.column( iid ).data().sum();
    //         // alert(iid);
    //         $( api.column( iid ).footer() ).html(csum.toFixed(3));
    //     });
    // }
