...
class Vehiclerunning extends CI_Controller {
	...
	public function delete($id)
	{
		if($this->_role != 1)
		{
			$this->session->set_flashdata('error', 'Only admin users can delete vehicle running entries.');
			redirect('vehicles-running-list');
		}
		...
		$this->session->set_flashdata('success', 'Vehicle Running entry deleted successfully');
		redirect('vehicles-running-list');
	}
