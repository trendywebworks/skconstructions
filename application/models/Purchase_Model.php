...
class Purchase_Model extends MY_Model
{
	...
	public function getAllPurchasesList($limit='')
	{
		...
		$select = "$this->_table.id,$this->_table.created_at,$this->_table.entry_date,$this->_suppliers_table.firm_name as firm_name,CONCAT('₹', FORMAT(SUM($this->_purchase_details_table.subtotal), 0)) AS bill_amount,CONCAT('₹', FORMAT(SUM($this->_purchase_details_table.gst_amount), 0)) AS gst_amount,
    CONCAT('₹', format(total_amount,0)) as total_amount,SUM($this->_purchase_details_table.quantity) AS quantity,DATE_FORMAT($this->_table.purchase_date,'%d-%m-%Y') as purchase_date,$this->_table.remarks,$this->_table.status";
