...
class VehicleRunning_Model extends MY_Model
{
    ...
    2)
) AS expenditure, CONCAT('₹', format($this->_table.income,2)) as income, CONCAT('₹', format($this->_table.balance,2)) as balance,$this->_table.remarks,$this->_table.status";
	...
	public function getAllVehicleRunningKmById($id)
	{
		...
		$select = "$this->_table.id,$this->_table.created_at,$this->_table.entry_date,$this->_vehicle_table.vehicle_name as vehicle_name,$this->_vehicle_table.vehicle_no,$this->_table.party_name,$this->_table.start_km,$this->_table.end_km,$this->_table.total_km, $this->_table.particular,CONCAT('₹', format($this->_table.amount,2)) as amount, CONCAT('₹', format($this->_table.diesel_amount,2)) as diesel_amount, CONCAT('₹', format($this->_table.income,2)) as income, CONCAT('₹', format($this->_table.balance,2)) as balance,$this->_table.status,$this->_table.remarks";
		$result = $this->db->select($select)->from($this->_table)->join($this->_vehicle_table, "$this->_vehicle_table.id=$this->_table.vehicle_id", 'inner')->where($where)->order_by('id','DESC')->get();
