...
class Officeexpense_Model extends MY_Model
{
	...
	public function getAllOfficeExpensesList()
	{
		...
		$select = "$this->_table.id,$this->_table.created_at,$this->_table.entry_date,$this->_expense_type_table.expense_type as particular, CONCAT('₹', format(amount,2)),$this->_table.remarks";
	...
	public function getAllOfficeExpensesListById($id)
	{
		...
		$select = "$this->_table.id,$this->_table.created_at,$this->_table.entry_date,$this->_expense_type_table.expense_type as expense_name,$this->_office_staffs_table.name as staff_name, CONCAT('₹', format(amount,2)) as amount,$this->_table.status,$this->_table.remarks";
		$result = $this->db->select($select)->from($this->_table)->join($this->_expense_type_table, "$this->_expense_type_table.id=$this->_table.expense_type_id", 'inner')->join($this->_office_staffs_table, "$this->_office_staffs_table.id=$this->_table.staff_id", 'inner')->where($where)->order_by('id','DESC')->get();
